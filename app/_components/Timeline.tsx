      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <h4 className="font-serif font-bold text-foreground min-w-fit flex items-center gap-2">
            ...
            {viewMode === "week"
              ? `Week of ${timelineDays[0].toLocaleDateString()}`
            ...
            >
              ...
              <div className="p-3 space-y-2 min-h-24">
                {entryCount === 0 ? (
                  ...
                  entries.map((entry) => {
                    const isFocus = 'focus' in entry; 
                    // const isDecision = 'reason' in entry; 

                    return (
                      <div
                      key={entry.id}
                      onClick={() => setSelectedEntryId(entry.id)}
                      className={`text-xs p-2 rounded-md truncate font-medium transition-smooth hover:shadow-sm cursor-pointer ${isFocus
