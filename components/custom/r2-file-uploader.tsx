...
interface R2FileUploaderProps {
  ...
  onImageRemove?: () => void; // Called when existing image is removed
  imageSrc?: string | null; // URL of existing image to show in edit mode
  viewOnly?: boolean; // If true, hides edit/remove buttons
  className?: string;
}
...
export const R2FileUploader = forwardRef<R2FileUploaderRef, R2FileUploaderProps>(
  (props, ref) => {
    ...
    const resetFiles = useCallback(() => {
      ...
      // If we have an existing image, show it again when resetting
      if (props.imageSrc) {
        setShowExistingImage(true);
      }
    }, [files, onFilesChange, props.imageSrc]);
    ...
    const removeFile = useCallback((id: string) => {
      ...
      setError('');
      // If we removed the last file and have an existing image, show it again
      if (files.length <= 1 && props.imageSrc) {
        setShowExistingImage(true);
        props.onImageRemove?.();
      }
    ...
    return (
      <div className={cn('w-full space-y-4', className)}>
        ...
        {showExistingImage && props.imageSrc ? (
                ...
                <div className="absolute inset-0 bg-black/50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                  <div className="flex gap-2">
                    ...
                    <Button
                      ...
                      variant="destructive"
                      size="sm"
                      onClick={() => handleRemoveExistingImage(props.imageSrc)}
                      className="rounded-full"
                    >
